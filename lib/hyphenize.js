...
class Hyphenizor {
	...
	parse(done) {
		let func = this
		xml2js.parseString(this.str, (err, result) => {
//			console.log(util.inspect(result, { showHidden: true, depth: null }))
			func.doc = result
			func.walk(func.doc, {lang: null})
		...
		if (util.isArray(node)) {
			...
			for (let idx in node) {
				if (util.isString(node[idx])) {
					this.hyphenize(node, idx, opt)
				} else {
					u.vputs(1, `array ${idx}`)
					this.walk(node[idx], opt)
				}
			}
		} else {
			...
			if (node['$'] && node['$']['lang']) {
				...
				opt.lang = node['$']['lang'].trim()
			}
			this.hyphenize(node, '_', opt)
